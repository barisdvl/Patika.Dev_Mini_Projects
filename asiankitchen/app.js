...
let menuList = ["All", "Korea", "Japan", "China"];
menuList.forEach((menuName) => {
  btnContainer.innerHTML += createBtn(menuName);
});

const btnAll = document
  .querySelector("#All")
  .addEventListener("click", listAllMenu);
const btnKorea = document
  .querySelector("#Korea")
  .addEventListener("click", listKoreaMenu);
const btnJapan = document
  .querySelector("#Japan")
  .addEventListener("click", listJapanMenu);
const btnChina = document
  .querySelector("#China")
  .addEventListener("click", listChinaMenu);

//Butonların filtre atamalarının yapılması
function listAllMenu() {
  let allMenu = "";
  menu.map((item) => {
    allMenu += createItem(item);
  });
...
function listChinaMenu() {
  ...
  menu.map((item) => {
    if (item.category == "China") {
      chinaMenu += createItem(item);
    }
  });
  menuContainer.innerHTML = chinaMenu;
}

//Menüdeki her item ın yaratılması
